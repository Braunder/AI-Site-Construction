...
def _user_prompt(params: dict, assets: list[dict] | None = None) -> str:
    ...
    if assets:
        ...
        text += (
            "\n\nДоступные файлы пользователя (ссылайся относительными путями):\n"
            f"{lines}\n"
            "Используй подходящие изображения в галерее/секциях сайта."
        )
    return text
    ...
    if assets:
        ...
        if asset_images:
            system += (
                ...
                "[файл: <имя>]. Учитывай их СОДЕРЖИМОЕ при выборе места размещения "
